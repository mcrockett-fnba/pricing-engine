...
GO
CREATE PROCEDURE [bid].[SaveServicingCosts](@pkgPurchaseId INT) 
AS 
BEGIN;
	...
	BEGIN TRY;
		...
		SELECT 
			...
			, x.overallAverage AS avgServicingCosts
		INTO #loans
		...
		CROSS APPLY bid.servicingCosts(itin.status, 
													credit.score,
														ROUND(loans.interest_rate_current,7),
														ROUND(ltv.value,4),
														loans.current_balance, 
														ROUND(loans.interest_rate_current - tRates.Tnote_10_year,4),
